...
router.get('/api/subscriptions/candidates', requireAuth, async (req: Request, res: Response) => {
	try {
		return res.status(200).json(await SubscriptionServiceInstance.detectCandidates(req.user.id));
	} catch (error) {
		logger.error('Failed to detect subscription candidates', { error, userId: req.user.id });
		return res.status(500).json({ message: 'Failed to detect subscription candidates' });
	}
});
...
router.get('/api/subscriptions', requireAuth, async (req: Request, res: Response) => {
	try {
		...
		logger.error('Failed to fetch subscriptions', { error, userId: req.user.id });
		return res.status(500).json({ message: 'Failed to fetch subscriptions' });
	}
});

router.post('/api/subscriptions/candidates/:candidateKey/confirm', requireAuth, async (req: Request, res: Response) => {
	try {
		...
		logger.error('Failed to confirm subscription candidate', { error, userId: req.user.id });
		return res.status(500).json({ message: 'Failed to confirm subscription candidate' });
	}
});

router.post('/api/subscriptions/candidates/:candidateKey/dismiss', requireAuth, async (req: Request, res: Response) => {
	try {
		return res.status(201).json(await SubscriptionServiceInstance.dismissCandidate(req.user.id, req.params.candidateKey));
	} catch (error) {
		if (error instanceof AppError) return res.status(error.statusCode).json({ message: error.message });
		logger.error('Failed to dismiss subscription candidate', { error, userId: req.user.id });
		return res.status(500).json({ message: 'Failed to dismiss subscription candidate' });
	}
});
...
router.patch('/api/subscriptions/:id', requireAuth, async (req: Request, res: Response) => {
	try {
...
router.delete('/api/subscriptions/:id', requireAuth, async (req: Request, res: Response) => {
	try {
		...
		if (error instanceof AppError) return res.status(error.statusCode).json({ message: error.message });
		logger.error('Failed to delete subscription', { error, userId: req.user.id });
